...
function formatDescrition(description){
...
function setlist(list) {
   ...
   for (var key in list) {
       table += '<tr>\n' +
           '          <td>'+formatDescrition(list[key].description)+'</td>\n' +
...
function validation(){
    ...
    if (description === ""){
        errors += '<p>Informe a descriçaõ</p>';
    }
    if (amount <= 0){
...
function saveListStorage(){
    //codigo para atualizar o Storage
    var jsonStr = JSON.stringify(list);
    localStorage.setItem("list",jsonStr);
}

function initListStorage(){
    //codigo para inicializar o Storage
